...
export const normalized = (headings: Heading[]): Heading[] => {
	...
	if (otherRoot >= 1) {
		// Increment and clamp headings to maximum under other roots
		for (let i = otherRoot; i < headings.length; i++) {
			const heading = headings[i];
			headings[i] = {
				...heading,
				level: heading.level + 1 <= 6 ? heading.level + 1 : 6,
			};
		}
	}
	return headings;
...
export const tableOfContents = (slugify: (token: string) => string) => (
	...
	const items: Heading[] = normalized(headings(slugify)(mainContents));
